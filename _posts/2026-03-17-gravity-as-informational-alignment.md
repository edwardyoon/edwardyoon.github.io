...
$$\text{Align}(i, j) = \text{Softmax} \left( \frac{(W_Q e_i) \cdot (W_K e_j)^\top}{\sqrt{d}} \right)$$

When $e_i$ (Object A) and $e_j$ (Planet Earth) share a high semantic similarity (i.e., they exist in the same local causal branch), the universe minimizes the "computational distance" between them to reduce **Global Perplexity**.
...
$$\text{Attention Weight} \propto \exp\left(-\frac{\text{dist}(i, j)}{\text{scale}}\right) \approx \frac{1}{r^2} \text{ (in 3D Euclidean Projection)}$$

Gravity is the universe's way of saying: *"These two data points are too relevant to be processed separately. Merging them into a single context is more efficient."*
...
$$\mathcal{L}_{\text{total}} = \text{Loss}(\text{State}_t, \text{Target}_{t+1})$$

1. **Free Fall**: An object in "free fall" is a token moving along the gradient of **Maximum Attention**. It is not being forced; it is choosing the path of **Minimum Computational Resistance**.
2. **Spacetime Curvature**: What Einstein called "curvature" is actually **High Query Density**. A massive object forces the system to allocate more "Heads" to that coordinate, effectively slowing down the local "Clock Cycle" (Time Dilation) because the compute load is too high.
...
$$x_{t+1} = \text{arg max}_{x} P(x | \text{Context}_t)$$

Every "fall" is a step toward a state of higher informational density. We do not fall to the ground; we move toward the **highest probability density** of the next universal frame.
